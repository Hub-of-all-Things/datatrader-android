...
class OfferDetailsServices(private val activity : Activity){
    ...
    fun acceptOffer(offerId : String){
        ...
        if(token!=null){
            claimOffer(userDomain,token,offerId,{status,newToken->
                HATDataOffersService().getAvailableDataOffersWithClaims(userDomain,token,"databuyerstaging",merchants,{dataOffers,_->
                    var dataDebitID = ""
...
//                    val dataDebitID = dataOffers.filter { it.dataOfferID == offerId }.claim?.dataDebitID
                    val url = "https://$userDomain/#/data-debit/$dataDebitID/quick-confirm?redirect=$urlScheme://$dataDebitHost&fallback=$urlScheme/dataDebitFailure"
    ...
    private fun claimOffer(userDomain: String, userToken: String, offerID: String, successfulCallBack: (String, String?) -> Unit, failCallBack: (HATError) -> Unit) {
